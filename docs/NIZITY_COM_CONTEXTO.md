...
| D13 | 2026-09-24 | Orbe vivo: o Núcleo envia só o nome do estado (13 estados) a um intermediário (proposta: Cloudflare Worker), com token secreto; o site lê e anima. Sem sinal por 3 min → demonstração. O visual do orbe é só cosmético e fica em `assets/orb.js`. Fundador escolheu "tempo real puro"; o Claude recomendou atraso/demonstração por privacidade (expõe rotina). | **Contrato aceito pelo Núcleo; Worker não construído** |
| D14 | 2026-09-24 | Hospedagem: **Cloudflare Pages** a partir do branch `main` (DNS já na Cloudflare, aceita `_headers` de segurança, mesmo lugar do futuro Worker do orbe). `main` = produção; trabalho novo por branch + PR. | Decidido pelo fundador ("publicar"); configuração no painel pendente |
